    ...
    return (
        <div className="flex flex-col items-center h-screen px-3 pt-16 pb-4 overflow-hidden">
            ...
            <div className="fixed top-0 left-0 w-full z-50 bg-[#050510]/90 backdrop-blur-xl border-b border-white/5">
                ...
                <div className="px-3 md:px-10 py-2 md:py-2.5 flex justify-between items-center">
                    <div className="flex items-center gap-1.5 md:gap-2 max-w-[45%]">
                        <User className="w-3 h-3 md:w-3.5 md:h-3.5 text-primary shrink-0" />
                        <div className="flex flex-col overflow-hidden">
                            <span className="text-[7px] md:text-[8px] font-black uppercase tracking-widest text-slate-500 leading-none mb-0.5">Identity</span>
                            <span className="text-[9px] md:text-[11px] font-bold text-white leading-none truncate">{userName || 'GUEST_PROTO'}</span>
                        </div>
                    </div>

                    <div className="flex items-center gap-2 max-w-[50%] justify-end">
                        <span className="text-[7px] md:text-[9px] font-black uppercase tracking-[0.1em] md:tracking-[0.2em] text-primary/60 truncate">{quizName || 'GENERAL_MODULE'}</span>
                        <span className="text-[8px] md:text-[9px] font-bold text-slate-600 bg-white/5 px-1.5 md:px-2 py-0.5 rounded-full shrink-0">{questionIndex + 1}/{questions.length}</span>
                    </div>
                </div>
            </div>
            ...
            <div className="w-full max-w-2xl flex-1 flex flex-col justify-center gap-4 md:gap-6 min-h-0">
                <AnimatePresence mode="wait">
                    <motion.div
                        ...
                        className="p-4 md:p-7 glass-card rounded-[1.5rem] md:rounded-[2rem] relative overflow-hidden flex flex-col min-h-0 shadow-2xl"
                    >
                        ...
                        <div className="relative z-10 flex flex-col min-h-0">
                            <div className="flex items-center gap-2 mb-2 md:mb-3">
                                <span className="w-3 md:w-4 h-[1px] bg-primary/40" />
                                <span className="text-[7px] md:text-[8px] font-black uppercase tracking-[0.3em] text-primary">Active_Channel</span>
                            </div>

                            <h2 className="text-lg md:text-2xl lg:text-3xl font-bold mb-4 md:mb-5 leading-tight tracking-tight text-white/90">
                                {question.question}
                            </h2>
